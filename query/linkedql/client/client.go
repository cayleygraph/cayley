...
// As As assigns the resolved values of the from step to a given name. The name can be used with the Select and Documents steps to retreive the values or to return to the values in further steps with the Back step. It resolves to the values of the from step.
func (p Path) As(name string) Path {
	return Path{"@type": "http://cayley.io/linkedql#As", "from": p, "http://cayley.io/linkedql#name": name}
}

// Back Back resolves to the values of the previous the step or the values assigned to name in a former step.
func (p Path) Back(name string) Path {
	return Path{"@type": "http://cayley.io/linkedql#Back", "from": p, "http://cayley.io/linkedql#name": name}
}

// Count Count resolves to the number of the resolved values of the from step
func (p Path) Count() Path {
	return Path{"@type": "http://cayley.io/linkedql#Count", "from": p}
}

// Difference Difference resolves to all the values resolved by the from step different then the values resolved by the provided steps. Caution: it might be slow to execute.
...
// Filter Apply constraints to a set of nodes. Can be used to filter values by range or match strings.
func (p Path) Filter() Path {
	return Path{"@type": "http://cayley.io/linkedql#Filter", "from": p}
}

// Follow The way to use a path prepared with Morphism. Applies the path chain on the morphism object to the current path. Starts as if at the g.M() and follows through the morphism path.
...
// Has Filter all paths which are, at this point, on the subject for the given predicate and object, but do not follow the path, merely filter the possible paths. Usually useful for starting with all nodes, or limiting to a subset depending on some predicate/value pair.
...
// HasReverse The same as Has, but sets constraint in reverse direction.
...
// Intersect Intersect resolves to all the same values resolved by the from step and the provided steps.
...
// Is Is resolves to all the values resolved by the from step which are included in provided values.
...
// Labels Get the list of inbound and outbound quad labels
func (p Path) Labels() Path {
	return Path{"@type": "http://cayley.io/linkedql#Labels", "from": p}
}

// Limit Limit a number of nodes for current path.
func (p Path) Limit() Path {
	return Path{"@type": "http://cayley.io/linkedql#Limit", "from": p}
}

// Order Order sorts the results in ascending order according to the current entity / value
...
// Placeholder Placeholder is like Vertex but resolves to the values in the context it is placed in. It should only be used where a PathStep is expected and can't be resolved on its own.
...
// PropertyNames Get the list of predicates that are pointing out from a node.
func (p Path) PropertyNames() Path {
	return Path{"@type": "http://cayley.io/linkedql#PropertyNames", "from": p}
}

// PropertyNamesAs Tag the list of predicates that are pointing out from a node.
...
// ReversePropertyNames Get the list of predicates that are pointing in to a node.
func (p Path) ReversePropertyNames() Path {
	return Path{"@type": "http://cayley.io/linkedql#ReversePropertyNames", "from": p}
}

// ReversePropertyNamesAs Tag the list of predicates that are pointing in to a node.
...
// Skip Skip a number of nodes for current path.
func (p Path) Skip() Path {
	return Path{"@type": "http://cayley.io/linkedql#Skip", "from": p}
}

// Union Return the combined paths of the two queries. Notice that it's per-path, not per-node. Once again, if multiple paths reach the same destination, they might have had different ways of getting there (and different tags).
...
// Unique Remove duplicate values from the path.
...
// Vertex Vertex resolves to all the existing objects and primitive values in the graph. If provided with values resolves to a sublist of all the existing values in the graph.
func Vertex(values []quad.Value) Path {
	return Path{"@type": "http://cayley.io/linkedql#Vertex", "http://cayley.io/linkedql#values": values}
}

// View View resolves to the values of the given property or properties in via of the current objects. If via is a path it's resolved values will be used as properties.
...
// ViewBoth ViewBoth is like View but resolves to both the object values and references to the values of the given properties in via. It is the equivalent for the Union of View and ViewReverse of the same property.
...
// ViewReverse The inverse of View. Starting with the nodes in `path` on the object, follow the quads with predicates defined by `predicatePath` to their subjects.
