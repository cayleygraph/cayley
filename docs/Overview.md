...
Examples for each backend:
  ...
  * `sql`: `./cayley init --db=sql --dbpath="user=user dbname=mydb password=mypassword sslmode=disable"` -- where the value of the `dbpath` flag is a connection string of parameters from [pq](https://github.com/lib/pq). See https://godoc.org/github.com/lib/pq for more information.
