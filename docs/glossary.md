...
4. From [Cayley godoc](https://godoc.org/github.com/cayleygraph/cayley/quad#Quad):
```go
type Quad struct {
...
2. `.v()` is for "Vertex" in Gizmo, and it is used like `pathObject = graph.Vertex([nodeId],[nodeId]...)`

### path
1. Path represents either a morphism (a pre-defined path stored for later use), or a concrete path, consisting of a morphism and an underlying QuadStore. [source](https://godoc.org/github.com/cayleygraph/cayley/graph/path#Path)
2. Paths are just a set of helpers to build a query, but they are not that good for building something more complex. You can try using Shapes for this - it will give you a full control of what the query actually does. [source](https://discourse.cayley.io/t/a-variety-of-questions/1183/2)
3. Underlying code:
  ```go
  type Path struct {
      stack       []morphism
...
### link
1. Another name for a quad?

### direction
1. Direction specifies an edge's type. [source](https://godoc.org/github.com/cayleygraph/cayley/quad#Direction)
```go
const (
    Any Direction = iota
    Subject
...
```
2. “[In RDF stores predicates are vertices as well](https://discourse.cayley.io/t/glossary-of-terms/180/7)”, meaning the connection between any two nodes is itself a node.     When traversing a graph by following connections, you could get the list of all quads your "current node" is a member of (as a Subject, for example). To then move to another node or set of nodes, you could pick a "direction" to move in, (say, Predicate) and thereby acquire a new set of nodes which cohabited quads with your original node.
